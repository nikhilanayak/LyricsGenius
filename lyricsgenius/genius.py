...
class Genius(API, PublicAPI):
    ...
    def lyrics(self, song_id=None, song_url=None, remove_section_headers=False):
        ...
        res = self._make_request(path, web=True).replace('<br/>', '\n')
        html = BeautifulSoup(
            res,
            "html.parser"
        )
        ...
        try:
            res = re.search('tags..:(.*?)]', res).group(0).replace("\\", "")
            res = res[6:]
            tags = json.loads(res)
        except:
            print(sys.exc_info())
        ...
        while next_page:
            ...
            for track in tracks_list['tracks']:
                ...
                if (song_info['lyrics_state'] == 'complete'
                    ...
                    song_lyrics, tags = self.lyrics(song_url=song_info['url'])
                    print(tags)
                else:
                    song_lyrics = ""

                track = Track(self, track, song_lyrics, tags=tags)
                tracks.append(track)
    ...
    def search_song(self, title=None, artist="", song_id=None,
        ...
        if (song_info['lyrics_state'] == 'complete'
                and not song_info.get('instrumental')):
            lyrics, tags = self.lyrics(song_url=song_info['url'])
        else:
            lyrics = ""
        ...
        song = Song(self, song_info, lyrics, tags=tags)
        if self.verbose:
            print('Done.')
        return song
